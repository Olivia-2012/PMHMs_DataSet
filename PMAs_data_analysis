...
predata = pd.read_excel('D:\Doctor\PMHMs-DataSet\data_new\PMAs.xlsx')


# 删除PMAs列下值为0的行
predata = predata[predata['PMAs'] != 0]

# 要剔除异常值的列
columns_to_process = ['PMAs', 'indSO2', 'indNOx', 'carNOx', 'carSmoke', 'pop',  'temp', 'rh', 'sd', 'wsp', 'preci']
...
# 使用自定义数据范围和标签进行分箱
data['PMAs_binned_labels'] = pd.cut(data['PMAs'], bins=bin_ranges, labels=bin_labels)

# 统计每个分箱中的数据数量
bin_counts = data['PMAs_binned_labels'].value_counts()
...
# 计算每个分箱的均值和标准差
bin_means = data.groupby('PMAs_binned_labels')['PMAs'].transform('mean')
bin_std = data.groupby('PMAs_binned_labels')['PMAs'].transform('std')
...
for index, row in data.iterrows():
    ...
    pmAs_estimates.append(estimate)

data['PMAs_estimate'] = pmAs_estimates

# 输出每行的估计值
for index, row in data.iterrows():
    print(f"真实值: {row['PMAs']}, 行索引: {index}, 分箱标签: {row['PMAs_binned_labels']}, 估计值: {row['PMAs_estimate']}")

# 分割自变量和因变量
X = data[['indSO2', 'indNOx', 'carNOx', 'carSmoke', 'pop', 'temp', 'rh', 'sd', 'wsp', 'preci']]
y = data['PMAs_binned_labels']
...
def generate_random_estimate(row):
    if pd.isna(row['PMAs_estimate']):
        bin_label = row['PMAs_binned_labels']
        ...
        return row['PMAs_estimate']

# 计算每个分箱的均值和标准差
bin_means = final_data.groupby('PMAs_binned_labels')['PMAs'].mean()
bin_std = final_data.groupby('PMAs_binned_labels')['PMAs'].std()

# 使用 apply 方法为 PMAs_estimate 列的空值生成随机估计值
final_data['PMAs_estimate'] = final_data.apply(generate_random_estimate, axis=1)

final_data.to_excel('As_expanded_data2.xlsx', index=False)

expanded_data = pd.read_excel("D:\Doctor\PMHMs-DataSet\data_new\As_expanded_data2.xlsx")

expanded_data.head()

# 将 PMAs_all 转换为以t为单位
expanded_data['PMAs_all'] = expanded_data['PMAs_all'] / 1e3
...
plt.hist(expanded_data[expanded_data['is_real']]['PMAs_all'], bins=10, color='green', alpha=0.5, label='PMAs (Real)', 
         edgecolor='black', hatch='//')
plt.xlabel('Real PMAs values (t)')
plt.ylabel('Frequency')
plt.xlim([min(expanded_data['PMAs_all']), max(expanded_data['PMAs_all'])])
plt.ylim([0, 160])

plt.subplot(132)  # 一行三列中的第二个图
plt.hist(expanded_data[~expanded_data['is_real']]['PMAs_all'], bins=9, color='blue', alpha=0.5, label='PMAs (Synthetic)', 
         edgecolor='black', hatch='\\')
plt.xlabel('Synthetic PMAs values (t)')
plt.xlim([min(expanded_data['PMAs_all']), max(expanded_data['PMAs_all'])])
plt.ylim([0, 160])  

plt.subplot(133)  # 一行三列中的第三个图
plt.hist(expanded_data['PMAs_all'], bins=10, color='yellow', alpha=0.5, label='PMAs (All)', edgecolor='black', hatch='--')
plt.xlabel('Total PMAs values (t)')
plt.xlim([min(expanded_data['PMAs_all']), max(expanded_data['PMAs_all'])])
plt.ylim([0, 160])  
...
# 列出需要进行对数转换的列
log_columns = ['PMAs_all', 'indSO2', 'indNOx', 'carNOx', 'carSmoke', 'pop', 'sd', 'wsp', 'preci', 'temp', 'rh']
...
has_nan = expanded_data['PMAs_all_log'].isna().any()

if has_nan:
    print("Column 'PMAs_all_log' contains NaN values.")
else:
    print("Column 'PMAs_all_log' does not contain NaN values.")

# 删除包含空值的行
expanded_data.dropna(subset=['PMAs_all_log'], inplace=True)
...
model_params_As = pd.DataFrame(columns=["Model", "Parameters"])
...
for model in models:
    for _ in range(n_repeats):
        ...
        model_params_As = model_params_As.append({"Model": type(model).__name__, "Parameters": model_params}, ignore_index=True)
...
# 保存最佳模型到文件
joblib.dump(best_model, 'best_model_rmse_As.joblib')
...
# 保存模型参数到Excel文件
model_params_As.to_excel("model_parameters_As.xlsx", index=False)
...
# 保存为Excel文件
# average_metrics.to_excel('As_model_metrics_averages.xlsx', index=False)
# metrics_df.to_excel('As_model_metrics.xlsx', index=False)

# 加载保存的最佳模型
# mymodel = joblib.load('best_model_rmse_As.joblib')
...
# 创建DataFrame
result_df = pd.DataFrame(result_dict_As)

# 将DataFrame保存到Excel文件
result_df.to_excel("PICP_results_As.xlsx", index=False)
...
# 将预测结果添加到新数据中
data_predict['RF_PMAs_2015'] = y_pred

# 打印包含预测值的新数据
data_predict.to_excel('Predicted_PMAs_2015.xlsx', index=False)
