...
predata = pd.read_excel('D:\Doctor\PMHMs-DataSet\data_new\PMCd.xlsx')


# 删除PMCd列下值为0的行
predata = predata[predata['PMCd'] != 0]

# 要剔除异常值的列
columns_to_process = ['PMCd', 'indSO2', 'indNOx', 'carNOx', 'carSmoke', 'pop',  'temp', 'rh', 'sd', 'wsp', 'preci']
...
# 使用自定义数据范围和标签进行分箱
data['PMCd_binned_labels'] = pd.cut(data['PMCd'], bins=bin_ranges, labels=bin_labels)

# 统计每个分箱中的数据数量
bin_counts = data['PMCd_binned_labels'].value_counts()
...
# 计算每个分箱的均值和标准差
bin_means = data.groupby('PMCd_binned_labels')['PMCd'].transform('mean')
bin_std = data.groupby('PMCd_binned_labels')['PMCd'].transform('std')
...
data['PMCd_estimate'] = pmcd_estimates

# 输出每行的估计值
for index, row in data.iterrows():
    print(f"真实值: {row['PMCd']}, 行索引: {index}, 分箱标签: {row['PMCd_binned_labels']}, 估计值: {row['PMCd_estimate']}")

# 分割自变量和因变量
X = data[['indSO2', 'indNOx', 'carNOx', 'carSmoke', 'pop', 'temp', 'rh', 'sd', 'wsp', 'preci']]
y = data['PMCd_binned_labels']
...
def generate_random_estimate(row):
    if pd.isna(row['PMCd_estimate']):
        bin_label = row['PMCd_binned_labels']
        ...
        return row['PMCd_estimate']

# 计算每个分箱的均值和标准差
bin_means = final_data.groupby('PMCd_binned_labels')['PMCd'].mean()
bin_std = final_data.groupby('PMCd_binned_labels')['PMCd'].std()

# 使用 apply 方法为 PMAs_estimate 列的空值生成随机估计值
final_data['PMCd_estimate'] = final_data.apply(generate_random_estimate, axis=1)

final_data.to_excel('Cd_expanded_data2.xlsx', index=False)

expanded_data = pd.read_excel("D:\Doctor\PMHMs-DataSet\data_new\Cd_expanded_data2.xlsx")

expanded_data.head()

# 将 PMCd_all 转换为以t为单位
expanded_data['PMCd_all'] = expanded_data['PMCd_all'] / 1e3
...
plt.hist(expanded_data[expanded_data['is_real']]['PMCd_all'], bins=10, color='green', alpha=0.5, label='PMCd (Real)', 
         edgecolor='black', hatch='//')
plt.xlabel('Real PMCd values (t)')
plt.ylabel('Frequency')
plt.xlim([min(expanded_data['PMCd_all']), max(expanded_data['PMCd_all'])])
plt.ylim([0, 160])

plt.subplot(132)  # 一行三列中的第二个图
plt.hist(expanded_data[~expanded_data['is_real']]['PMCd_all'], bins=9, color='blue', alpha=0.5, label='PMCd (Synthetic)', 
         edgecolor='black', hatch='\\')
plt.xlabel('Synthetic PMCd values (t)')
plt.xlim([min(expanded_data['PMCd_all']), max(expanded_data['PMCd_all'])])
plt.ylim([0, 160])  

plt.subplot(133)  # 一行三列中的第三个图
plt.hist(expanded_data['PMCd_all'], bins=10, color='yellow', alpha=0.5, label='PMCd (All)', edgecolor='black', hatch='--')
plt.xlabel('Total PMCd values (t)')
plt.xlim([min(expanded_data['PMCd_all']), max(expanded_data['PMCd_all'])])
plt.ylim([0, 160])  
...
# 列出需要进行对数转换的列
log_columns = ['PMCd_all', 'indSO2', 'indNOx', 'carNOx', 'carSmoke', 'pop', 'sd', 'wsp', 'preci', 'temp', 'rh']
...
has_nan = expanded_data['PMCd_all_log'].isna().any()

if has_nan:
    print("Column 'PMCd_all_log' contains NaN values.")
else:
    print("Column 'PMCd_all_log' does not contain NaN values.")

# 删除包含空值的行
expanded_data.dropna(subset=['PMCd_all_log'], inplace=True)
...
model_params_Cd = pd.DataFrame(columns=["Model", "Parameters"])
...
for model in models:
    for _ in range(n_repeats):
        ...
        model_params_Cd = model_params_Cd.append({"Model": type(model).__name__, "Parameters": model_params}, ignore_index=True)
...
# 保存最佳模型到文件
joblib.dump(best_model, 'best_model_rmse_Cd.joblib')
...
# 保存模型参数到Excel文件
model_params_Cd.to_excel("model_parameters_Cd.xlsx", index=False)
...
# 保存为Excel文件
# average_metrics.to_excel('Cd_model_metrics_averages.xlsx', index=False)
# metrics_df.to_excel('Cd_model_metrics.xlsx', index=False)

# 加载保存的最佳模型
# mymodel = joblib.load('best_model_rmse_Cd.joblib')
...
result_df.to_excel("PICP_results_Cd.xlsx", index=False)
...
# 将预测结果添加到新数据中
data_predict['RF_PMCd_2015'] = y_pred

# 打印包含预测值的新数据
data_predict.to_excel('Predicted_PMCd_2015.xlsx', index=False)
