...
predata = pd.read_excel('D:\Doctor\PMHMs-DataSet\data_new\PMPb.xlsx')


# 删除PMPb列下值为0的行
predata = predata[predata['PMPb'] != 0]

# 要剔除异常值的列
columns_to_process = ['PMPb', 'indSO2', 'indNOx', 'carNOx', 'carSmoke', 'pop',  'temp', 'rh', 'sd', 'wsp', 'preci']
...
# 自定义分箱的数据范围和标签
bin_ranges = [38, 800, 1500, 2500, 5000, 10000, 15000, 30000, 60000, 85374] # Pb数据扩充范围
bin_labels = ['0', '1', '2', '3', '4', '5', '6', '7', '8']

# 使用自定义数据范围和标签进行分箱
data['PMPb_binned_labels'] = pd.cut(data['PMPb'], bins=bin_ranges, labels=bin_labels)

# 统计每个分箱中的数据数量
bin_counts = data['PMPb_binned_labels'].value_counts()
...
# 计算每个分箱的均值和标准差
bin_means = data.groupby('PMPb_binned_labels')['PMPb'].transform('mean')
bin_std = data.groupby('PMPb_binned_labels')['PMPb'].transform('std')
...
for index, row in data.iterrows():
    ...
    pmPb_estimates.append(estimate)

data['PMPb_estimate'] = pmPb_estimates

# 输出每行的估计值
for index, row in data.iterrows():
    print(f"真实值: {row['PMPb']}, 行索引: {index}, 分箱标签: {row['PMPb_binned_labels']}, 估计值: {row['PMPb_estimate']}")

# 分割自变量和因变量
X = data[['indSO2', 'indNOx', 'carNOx', 'carSmoke', 'pop', 'temp', 'rh', 'sd', 'wsp', 'preci']]
y = data['PMPb_binned_labels']
...
def generate_random_estimate(row):
    if pd.isna(row['PMPb_estimate']):
        bin_label = row['PMPb_binned_labels']
        ...
        return row['PMPb_estimate']

# 计算每个分箱的均值和标准差
bin_means = final_data.groupby('PMPb_binned_labels')['PMPb'].mean()
bin_std = final_data.groupby('PMPb_binned_labels')['PMPb'].std()

# 使用 apply 方法为 PMPb_estimate 列的空值生成随机估计值
final_data['PMPb_estimate'] = final_data.apply(generate_random_estimate, axis=1)

final_data.to_excel('Pb_expanded_data2.xlsx', index=False)

expanded_data = pd.read_excel("D:\Doctor\PMHMs-DataSet\data_new\Pb_expanded_data2.xlsx")

expanded_data.head()

# 将 PMPb_all 转换为以t为单位
expanded_data['PMPb_all'] = expanded_data['PMPb_all'] / 1e3
...
plt.hist(expanded_data[expanded_data['is_real']]['PMPb_all'], bins=10, color='green', alpha=0.5, label='PMPb (Real)', 
         edgecolor='black', hatch='//')
plt.xlabel('Real PMPb values (t)')
plt.ylabel('Frequency')
plt.xlim([min(expanded_data['PMPb_all']), max(expanded_data['PMPb_all'])])
plt.ylim([0, 160])

plt.subplot(132)  # 一行三列中的第二个图
plt.hist(expanded_data[~expanded_data['is_real']]['PMPb_all'], bins=9, color='blue', alpha=0.5, label='PMPb (Synthetic)', 
         edgecolor='black', hatch='\\')
plt.xlabel('Synthetic PMPb values (t)')
plt.xlim([min(expanded_data['PMPb_all']), max(expanded_data['PMPb_all'])])
plt.ylim([0, 160])  

plt.subplot(133)  # 一行三列中的第三个图
plt.hist(expanded_data['PMPb_all'], bins=10, color='yellow', alpha=0.5, label='PMPb (All)', edgecolor='black', hatch='--')
plt.xlabel('Total PMPb values (t)')
plt.xlim([min(expanded_data['PMPb_all']), max(expanded_data['PMPb_all'])])
plt.ylim([0, 160])  
...
# 列出需要进行对数转换的列
log_columns = ['PMPb_all', 'indSO2', 'indNOx', 'carNOx', 'carSmoke', 'pop', 'sd', 'wsp', 'preci', 'temp', 'rh']

# 进行对数转换
for col in log_columns:
    expanded_data[col + '_log'] = np.log(expanded_data[col])

# 检查 'PMPb_log' 列是否包含 NaN 值
has_nan = expanded_data['PMPb_all_log'].isna().any()

if has_nan:
    print("Column 'PMPb_all_log' contains NaN values.")
else:
    print("Column 'PMPb_all_log' does not contain NaN values.")

# 删除包含空值的行
expanded_data.dropna(subset=['PMPb_all_log'], inplace=True)
...
model_params_Pb = pd.DataFrame(columns=["Model", "Parameters"])
...
for model in models:
    for _ in range(n_repeats):
        ...
        model_params_Pb = model_params_Pb.append({"Model": type(model).__name__, "Parameters": model_params}, ignore_index=True)
...
# 保存最佳模型到文件
joblib.dump(best_model, 'best_model_rmse_Pb.joblib')
...
# 保存模型参数到Excel文件
model_params_Pb.to_excel("model_parameters_Pb.xlsx", index=False)
...
# 保存为Excel文件
# average_metrics.to_excel('Pb_model_metrics_averages.xlsx', index=False)
# metrics_df.to_excel('Pb_model_metrics.xlsx', index=False)

# 加载保存的最佳模型
# mymodel = joblib.load('best_model_rmse_Pb.joblib')
...
# 创建DataFrame
result_df = pd.DataFrame(result_dict_Pb)

# 将DataFrame保存到Excel文件
result_df.to_excel("PICP_results_Pb.xlsx", index=False)
...
# 将预测结果添加到新数据中
data_predict['RF_PMPb_2015'] = y_pred

# 打印包含预测值的新数据
data_predict.to_excel('Predicted_PMPb_2015.xlsx', index=False)
